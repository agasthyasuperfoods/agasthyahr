...
export default function ResetPassword() {
  ...
  const submit = async (e) => {
    e.preventDefault();
    if (!token) {
      Swal.fire({ icon: "error", title: "Invalid link", text: "Missing or malformed token." });
      return;
    }
    if (pwd.length < 8) {
          ...
          <form onSubmit={submit} className="mt-4 space-y-4">
            <div>
              ...
              <div className="relative mt-1">
                ...
                />
                <button type="button" onClick={() => setShow((v) => !v)} className="absolute inset-y-0 right-0 px-3 text-gray-500">
                  {show ? "Hide" : "Show"}
                </button>
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={saving}
              className="w-full inline-flex items-center justify-center rounded-lg bg-[#C1272D] text-white font-medium py-2.5 hover:bg-[#a02125] disabled:opacity-60"
            >
