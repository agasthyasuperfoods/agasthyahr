...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!token || !pwd) return res.status(400).json({ error: "Missing token or password" });
    ...
    // Prefer token.aud if present; else decide based on 'to'
    ...
    const result = await pool.query(sql, [pwd, email, employeeid]);
    ...
    if (DEBUG) console.error("[reset/complete] error:", e?.message || e);
