  ...
  return (
    <div className="w-full mx-auto py-4 pb-[70px] px-2">
        ...
        <div className="overflow-x-auto rounded-lg shadow mb-3">
          <table className="min-w-full bg-white text-sm">
            ...
            <tbody>
              ...
              {!loading && data.length === 0 && (
                <tr><td colSpan={6} className="py-8 text-center text-gray-600">No attendance records.</td></tr>
              )}
              {data.map((row, idx) => {
                const textColor = STATUS_TEXT[row.status] || STATUS_TEXT.Default;
                return (
      ...
      <div className="w-full flex justify-between items-center mt-2 px-1">
        <button
          className="px-4 py-1 rounded-lg bg-gray-200 text-gray-700 text-xs font-bold border shadow-sm transition hover:bg-gray-300 disabled:opacity-50"
          disabled={page <= 1}
