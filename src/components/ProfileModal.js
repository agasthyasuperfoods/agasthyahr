...
export default function UsersTable() {
  ...
  const loadUsers = async () => {
    try {
      ...
      // Normalize ALL columns we have in DB
      const normalized = raw.map((u) => ({
        ...
        // We will NOT display the raw password for safety, only mask it in UI
  ...
  const filtered = useMemo(() => {
    ...
    return users.filter((u) => {
      const fields = [
        ...
        u?.designation ?? "",
        u?.reporting_to_id ?? "",
        // not including password in search
      ];
  ...
  return (
    <section>
      ...
      {/* Wide table with horizontal scroll so ALL columns fit */}
        ...
        <table className="min-w-[1600px] w-full text-xs">
          <thead className="bg-gray-50 sticky top-0 z-10">
            <tr className="text-left text-gray-600">
              ...
              <th className="px-3 py-2 border-b whitespace-nowrap">Role</th>
              ...
              <th className="px-3 py-2 border-b whitespace-nowrap">Password</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={15} className="px-3 py-6 text-center text-red-600">{error}</td>
              </tr>
            ) : filtered.length === 0 ? (
              <tr>
                <td colSpan={15} className="px-3 py-6 text-center text-gray-500">No matching users. Try adjusting your search.</td>
              </tr>
            ) : (
              pageSlice.map((u, i) => (
                <tr key={String(u.employeeid || i)} className="odd:bg-white even:bg-gray-50">
                  ...
                  <td className="px-3 py-2 border-t whitespace-nowrap">{u.role || "-"}</td>
                  ...
                  <td className="px-3 py-2 border-t whitespace-nowrap">{u.grosssalary ?? "-"}</td>
                  <td className="px-3 py-2 border-t whitespace-nowrap">{u.adhaarnumber ?? "-"}</td>
                  <td className="px-3 py-2 border-t whitespace-nowrap">{u.pancard ?? "-"}</td>
                  <td className="px-3 py-2 border-t">
                    <div className="max-w-[280px] truncate">{u.address ?? "-"}</div>
                  </td>
                  {/* Masked password */}
                  <td className="px-3 py-2 border-t whitespace-nowrap">{u.password ? "••••••" : "-"}</td>
...
function CreateUserModal({ onClose, onCreated }) {
  ...
  // Aadhaar & PAN (since you asked for ALL DB fields)
...
function UpdateUserModal({ data, onClose, onUpdated }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex itemsEND md:items-center justify-center" aria-modal="true" role="dialog">
      <div className="absolute inset-0 bg-black/40" onClick={onClose} aria-hidden="true" />
      <div className="relative w-full md:max-w-7xl bg-white border border-gray-200 rounded-t-2xl md:rounded-2xl shadow-xl p-6 m-0 md:m-4">
        <div className="flex items-center justify-between mb-2">
