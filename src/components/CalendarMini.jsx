...
// - Left: Search with typeahead (hard-close, keyboard nav, debounce)
...
// NOTE: We replaced the external CalendarMini with a local AttendanceCalendar below.
...
function AttendanceCalendar({ className = "" }) {
  const [statusMap, setStatusMap] = useState({}); // { 'YYYY-MM-DD': 'submitted' | 'not_submitted' }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        // Build all dates in this month that are <= today
        const runDates = [];
        ...
        // Hit the existing API for each date (Promise.all is OK for ~30 requests)
        const results = await Promise.all(
          runDates.map(async (ds) => {
            try {
              ...
              // hasData => submitted; otherwise not_submitted
              return [ds, j?.hasData ? "submitted" : "not_submitted"];
            } catch {
              // On error, treat as not submitted (safe fallback) for past days
    ...
    <div className={`rounded-xl border border-gray-200 bg-white shadow-sm p-3 ${className}`} style={{ width: 300 }}>
      ...
      <div className="grid grid-cols-7 gap-1 text-center">
        {cells.map((d, i) => {
          ...
          const ds = toISO(d);
          const status = statusMap[ds]; // undefined for future (no fetch)
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-gray-50">
        <AppHeader  currentPath={router.pathname}
          hrName={hrName}
        ...
        <div className="p-4 md:p-6 space-y-8">
          ...
          <section className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6">
            <div className="mb-4">
              <h3 className="text-base font-semibold text-gray-900">Team Directory</h3>
              <p className="text-xs text-gray-500 mt-0.5">
                Find an employee, view details, and manage assigned assets.
              </p>
            </div>
              ...
              <div className="lg:col-span-6">
                ...
                <div className="relative" ref={suggestRef}>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-gray-300 pl-9 pr-28 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-gray-200"
                  ...
                  <button
                    ...
                    onClick={doSearch}
                    disabled={searching}
                    className="absolute right-2 top-1/2 -translate-y-1/2 inline-flex items-center gap-1 rounded-lg bg-gray-900 px-3 py-1.5 text-sm font-medium text-white hover:bg-gray-800 disabled:opacity-60"
                  >
                    {searching ? (
                      <>
                  ...
                  {search ? (
                    <button
                      ...
                      className="absolute right-[86px] top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"
                      aria-label="Clear"
                      title="Clear"
                    >
                      ×
              ...
              <div className="lg:col-span-4">
                <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-3">
                  <div className="flex items-start gap-4">
                    <AttendanceCalendar />
                    {/* Legend (to the right on large, wraps under on small) */}
